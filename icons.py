...
# --- Hill with wind turbines (Knoten) ---
...
def shark(d,x,y,s):
    ...
    poly(d,[(x-s*.16,y-s*.40),(x+s*.10,y-s*.95),(x+s*.28,y-s*.34)],b)     # dorsal fin
    poly(d,[(x+s*.90,y+s*.06),(x+s*1.35,y-s*.32),(x+s*1.28,y+s*.40)],b)   # tail
    circ(d,x-s*.55,y-s*.10,s*.10,INK,INK,1)
    circ(d,x-s*.58,y-s*.13,s*.035,(255,255,255),None,0)
    d.arc([x-s*.86,y+s*.00,x-s*.36,y+s*.30],10,150,fill=INK,width=max(2,int(s*.06)))  # smile

# --- Home in the woods (Brandoberndorf) ---
def home(d,x,y,s):
